 ...
 * in `echoreel` and absurd in the app that stores a car's history.
 */
describe("el alcance de una frase", () => {
  ...
  it("se escribe delante y se lee de vuelta", () => {
    ...
    expect(text).toContain("- only in echoreel: Quieres una bandeja de audio.");
  ...
  it("se lee escrito de cualquier forma", () => {
    const leido = parseTaste("## frontend\n- Only  In  veloria :  Una cosa.\n");
    expect(leido.lines[0]).toMatchObject({ statement: "Una cosa.", scope: "veloria" });
  });
  ...
  it("la marca de citas sigue funcionando con alcance delante", () => {
    ...
    expect(leido).toMatchObject({ scope: "echoreel", citations: ["abc123"] });
  });
...
describe("el resumen que baja a cada proyecto", () => {
  const lines: TasteLine[] = [
    ...
    { topic: "frontend", statement: "De echoreel.", citations: [], scope: "echoreel" },
    { topic: "frontend", statement: "De veloria.", citations: [], scope: "veloria" },
  ];
  const profile = parseTaste(renderTaste(lines));

  it("un proyecto recibe lo global y lo suyo, y nada de los demás", () => {
    const bloque = tasteDigest(profile, Infinity, "echoreel");
    expect(bloque).toContain("Global.");
    expect(bloque).toContain("De echoreel.");
    expect(bloque).not.toContain("De veloria.");
  });

  it("un proyecto sin frases propias recibe solo lo global", () => {
    const bloque = tasteDigest(profile, Infinity, "otro");
    expect(bloque).toContain("Global.");
    expect(bloque).not.toContain("De echoreel.");
  });
  ...
  it("sin proyecto entra todo", () => {
    ...
    expect(bloque).toContain("De echoreel.");
    expect(bloque).toContain("De veloria.");
  });
});
