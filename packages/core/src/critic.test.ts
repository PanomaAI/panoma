...
/*
  The palette of `pocket_bot`, just as `readDesign` returns it today. It is the case that governs
 ...
 */
const POCKET_BOT = [
  color("#2196f3", 26, "lib/kiosk/widgets/message_input_field.dart"),
  color("#363636", 10),
  color("#000000", 8),
  color("#3c3c3c", 6),
  color("#e1306c", 4),
  color("#2195f3", 2, "lib/kiosk/widgets/kiosk_widget.dart"),
  color("#25d366", 2),
  color("#393939", 2),
  color("#3a3a3a", 2),
  color("#606060", 2),
...
describe("reviewProject · el proyecto contra su propia paleta", () => {
  it("denuncia el color suelto que se distingue en un dígito del que sí se usa", async () => {
    const findings = await reviewWith({ colors: POCKET_BOT });
    ...
    expect(findings[0]).toMatchObject({
      ...
      claim: "#2195f3",
      hint: "#2196f3",
      file: "lib/kiosk/widgets/kiosk_widget.dart",
    });
  });
  ...
  it("se calla con los grises vecinos: mueven los tres canales, o sea que se eligieron", async () => {
    ...
    const findings = await reviewWith({
      colors: POCKET_BOT.filter((entry) => entry.hex !== "#2195f3"),
    });
  ...
  it("con la huella truncada no se cuenta nada: dos apariciones pueden ser doscientas", async () => {
    const findings = await reviewWith({ colors: POCKET_BOT, truncated: true });
...
describe("reviewProject · las esquinas", () => {
  it("denuncia el radio suelto que a la vista es el mismo que el de siempre", async () => {
    // `pocket_bot`: `15px` in all the chat bubbles and a `16.0px` in a form.
    const findings = await reviewWith({
...
describe("reviewProject · enlaces que no llevan a ninguna parte", () => {
  it("denuncia el enlace roto del markdown y dice dónde vive uno que se llama igual", async () => {
    // It is the exact shape that the defect has on the author's disc: a tracking table in `kestrel`
...
describe("el silencio parcial se dice", () => {
  it("una huella truncada marca el informe, aunque el índice esté entero", async () => {
    // Measured in quiet_mark/frontend: the footprint came out truncated, the two drift checks
