 ...
 *
 * Requiring only `.git` left out a case that was indeed a project: `veloria-landing`, a page
 ...
 * folder — thus, an app called `veloria` appeared in the catalog as `mapbox-maps-flutter-main`,
...
export async function discoverProjects(root: string, maxDepth = 3): Promise<string[]> {
  ...
  async function scan(dir: string, depth: number): Promise<void> {
    ...
    if (kind === "git") {
      ...
      // was extra.
      //
      // Except when the container is named the same as one of its apps (`kestrel/kestrel`): there
      ...
      /*
        ...
        built inside. It appeared in the catalog as just another project, next to `veloria`,
