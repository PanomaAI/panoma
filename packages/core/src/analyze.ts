...
export async function analyzeProject(
  ...
  /*
    ...
    called `quiet_mark` whose README opens with “# Ledgerly” is called Ledgerly, and cataloging
    it as quiet_mark made its own author unable to find it. The folder remains a last resort: its
  ...
  /*
    ...
    you put in front of it the one that contains it: `echoreel/server` is «echoreel server».
  ...
  // `brand assets` talks about `sundeck`, which lives inside.
  ...
  const chosen =
    ...
    // a server usually carries the full product name. `echoreel/server` says «EchoReel» and it
    // was cataloged the same as the app: «EchoReel server» separates them.
  ...
  /*
    ...
    it — 'deskbot backend' — without touching the names that already identify something.
   */
