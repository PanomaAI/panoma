 ...
 * `-casa-apuntes` is placed in front of `-casa-otro` and the sample from the `limit` test comes
...
const CWD = "/casa/apuntes/apps/web";
...
describe("mineClaudeCode", () => {
  ...
  it("limit recorta la muestra y deja el embudo entero", async () => {
    ...
    const home = makeHome({
      "-casa-apuntes/sesion-1.jsonl": lines,
  ...
  it("el fixture sella la antigüedad a mano y no se fía del reloj", () => {
     ...
     */
    const home = makeHome({
      "-casa-apuntes/sesion-1.jsonl": [assistant("Hecho."), user("vale")],
      "-casa-otro/sesion-2.jsonl": [assistant("Hecho."), user("vale")],
    });
    const projects = join(home, ".claude", "projects");
    const primero = statSync(join(projects, "-casa-apuntes", "sesion-1.jsonl")).mtimeMs;
  ...
  it("cwdPrefix filtra por proyecto sin tocar el disco", async () => {
    const home = makeHome({
      "-casa-apuntes/sesion-1.jsonl": [
        assistant("Hecho."),
        user("de la web", { cwd: "/casa/apuntes/apps/web" }),
        assistant("Hecho."),
        user("de la cli", { cwd: "/casa/apuntes/apps/cli" }),
        assistant("Hecho."),
        // The neighbor with the same literal prefix: without checking the slash, it would match.
        user("del vecino", { cwd: "/casa/apuntes-viejo/apps/web" }),
        assistant("Hecho."),
        user("sin cwd", { cwd: undefined }),
      ],
    });

    const { reactions } = await mineClaudeCode({ home, cwdPrefix: "/casa/apuntes" });
  ...
  it("recorta la entrega a un extracto y la deja en una línea", async () => {
    ...
    const home = makeHome({
      "-casa-apuntes/sesion-1.jsonl": [assistant(`Primera línea\n\n${largo}`), user("quítalo")],
    });
  ...
  it("el recorte no parte un emoji por la mitad", async () => {
    ...
    const home = makeHome({
      "-casa-apuntes/sesion-1.jsonl": [assistant(entrega), user(reaccion)],
    });
  ...
  it("el recorte copia el trozo y no deja anclado el turno entero", async () => {
    ...
    const home = makeHome({ "-casa-apuntes/sesion-1.jsonl": lines });
 ...
 *
 * The real case that made it necessary: a transcript with `cwd` in `arca89/quiet_mark` in 1,095
 * turns while the files that the agent touched were **161 under `echoreel/` and one under
 * `quiet_mark/` **. The session was opened in one project and the work belonged to another, which
 * is normal when several live under the same parent folder. Consequence: a sentence about an audio
 * tray, learned while working in `echoreel`, ended up being credited to `Ledgerly` — and the
...
describe("de qué se hablaba, no dónde estaba el terminal", () => {
  ...
  it("la reacción sale con lo que el agente acababa de tocar", async () => {
    const home = makeHome({
      "-casa-uno/s.jsonl": [
        working(["/otro/echoreel/app/audio.dart"]),
    ...
    expect(reactions[0]!.paths).toEqual(["/otro/echoreel/app/audio.dart"]);
  });
  ...
  it("la ventana es todo lo hecho desde tu turno anterior, no el último mensaje", async () => {
    const home = makeHome({
      "-casa-dos/s.jsonl": [
        working(["/otro/echoreel/app/a.dart"], ""),
        working(["/otro/echoreel/app/b.dart"], ""),
    ...
    expect(reactions[0]!.paths).toEqual([
      "/otro/echoreel/app/a.dart",
      "/otro/echoreel/app/b.dart",
    ]);
  });
  ...
  it("una reacción anterior a tocar nada hereda las rutas de su sesión", async () => {
    const home = makeHome({
      "-casa-tres/s.jsonl": [
        ...
        working(["/otro/echoreel/app/audio.dart"], "Hecho."),
        user("perfecto"),
      ],
    });
  ...
  it("hereda primero lo más tocado", async () => {
    const home = makeHome({
      "-casa-cuatro/s.jsonl": [
        ...
        working(["/otro/echoreel/app/audio.dart"], ""),
        working(["/otro/echoreel/app/audio.dart"], ""),
        working(["/otro/quiet_mark/README.md"], "Hecho."),
        user("vale"),
      ],
    });

    const { reactions } = await mineClaudeCode({ home });
    expect(reactions[0]!.paths?.[0]).toBe("/otro/echoreel/app/audio.dart");
  });
            ...
            content: [
              {
                ...
                id: "toolu_1",
                name: "Bash",
                input: { command: "rm -rf /otro/echoreel/node_modules" },
              },
