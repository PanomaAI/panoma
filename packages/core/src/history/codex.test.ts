...
describe("mineCodex", () => {
  ...
  it("regla 3: el cwd del turno manda sobre el de la cabecera", async () => {
    const home = makeHome({
      [ROLLOUT]: [
        meta({ cwd: "/casa/apuntes" }),
        agent("Hecho en apuntes."),
        human("quítalo"),
 ...
 * failure — the terminal parked at `~/Documents/arca89` while the work occurred at
 * `arca89/quiet_mark`, which is another project in the catalog.
 *
...
describe("de qué proyecto era el trabajo", () => {
  it("saca el workdir que declara la herramienta, aunque el terminal esté en otro sitio", async () => {
    const home = makeHome({
      [ROLLOUT]: [
        meta(),
        exec("pnpm test", "/casa/apuntes/apps/web/interno"),
        agent("Hecho."),
    ...
    expect(reactions[0]?.paths).toEqual(["/casa/apuntes/apps/web/interno"]);
  });
  ...
  it("saca los ficheros que nombra un parche", async () => {
    const home = makeHome({
      [ROLLOUT]: [
        meta(),
        patch(["/casa/apuntes/uno.ts", "/casa/apuntes/dos.ts"]),
        agent("Hecho."),
        human("mal"),
      ],
    });
    ...
    const { reactions } = await mineCodex({ home });
    expect(reactions[0]?.paths).toEqual(["/casa/apuntes/uno.ts", "/casa/apuntes/dos.ts"]);
  });
  ...
  it("del comando en sí no sale ninguna ruta", async () => {
    const home = makeHome({
      [ROLLOUT]: [
        meta(),
        exec("cd /casa/inventada && rm -rf /casa/tampoco", "/casa/apuntes"),
        agent("Hecho."),
        human("no"),
      ],
    });
    ...
    const { reactions } = await mineCodex({ home });
    expect(reactions[0]?.paths).toEqual(["/casa/apuntes"]);
  });
