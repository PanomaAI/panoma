...
/*
  ...
  7. `cwd` is almost never the root of the project: what is measured is `apuntes/apps/web`,
  `quiet_mark/frontend`, `veloria/ios`, `.claude/worktrees/*`. It is returned **raw** and
  ...
  believing it alone. A real transcript from this machine: `cwd` = `arca89/quiet_mark` in 1,095
  turns, and the files that the agent touched in that same file were **161 under `echoreel/` and
  one under `quiet_mark/` **. The session was opened in one project and the work was from another,
  ...
  `cwd` is the fallback. The owner caught it while reading the review screen: a phrase from `echoreel`
  signed as from `Ledgerly`.
...
export interface Reaction {
   ...
   * `Documents/arca89/quiet_mark` in 1,095 turns, and the files that were touched in the entire
   * file were **161 under `echoreel/` and one under `quiet_mark/` **. The terminal was parked on
   ...
   * `echoreel` was attributed to `Ledgerly`, and the person caught it by reading the review
   * screen.
   *
...
async function mineTranscript(
  ...
  /*
    ...
    project. The entire session, on the other hand, touched 161 files of `echoreel` and one of
    `quiet_mark`. Asking the session responds correctly.
