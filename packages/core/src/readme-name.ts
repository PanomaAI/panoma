 ...
 *
 * The case that led to this function: a folder called `quiet_mark` whose README starts with "# Ledgerly —
 * the honest backtester for trading strategies." In the catalog it appeared as "quiet_mark," so
...
export function readmeName(
  ...
  /*
    ...
    «Ledgerly — the honest backtester for trading strategies» is the most common pattern of
    ...
    «quiet-mark» and «create-react-app» would be split at the hyphen of their own name. The colon
  ...
  /*
    ...
    `brand assets/README.md` starts with «# Sundeck», which is the app that lives in
    `brand assets/sundeck`. Taking it left two cards —«Sundeck» the container and «sundeck»
 ...
 *
 * `echoreel/server` appeared in the catalog as 'server', next to another 'server' from another
 ...
 *
 * `echoreel/server` → «echoreel server». Nothing else is touched: a project called `veloria`
 ...
 *
 * The case: `echoreel/server` has a README that says «EchoReel», so it was called EchoReel —
 * just like the app that lives in `echoreel/app`. Two almost identical cards for two different
 * things: a Flutter app and a Python server. «EchoReel server» separates them without taking away
