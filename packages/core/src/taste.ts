...
export interface TasteLine {
   ...
   * true in `echoreel` and absurd in the app that saves a car's history.
   *
   * By name and not by identity, because this is written in a file that the person opens and
   * edits: `only in veloria:` is read and corrected; `only in git:0516a71734…:` is not. A
...
/**
 * How to write and read the scope of a sentence: `- only in veloria: no soportas…`.
 *
 ...
 * annotations of everyone else inside: Ledgerly rules descending to `AGENTS.md` from a loose
