 ...
 * `kiosk_new copy 16(junio 3 2024)` with `kiosk_new`.
 */
const COPY_MARKERS = [
  /\bcop(y|ia)\b/gi,
...
function compare(a: Signature, b: Signature): { confidence: number; reason: string } | undefined {
  ...
  // remote, and date. Without this output, `veloria` and the landing would be marked as copies
  // of `kestrel` — eleven unrelated projects in a single family, connected by transitivity through
