...
async function writeCatalog(
  ...
  /*
    ...
    scan and false for a restricted one: `panoma scan ~/Desktop/menucard --save` analyzes a project,
    ...
    copies of `kiosk_new` started competing again as distinct projects on the cover.
 ...
 * It is what people mean when they write `/p/leaselab`.
...
async function assignSlugs(tx: Database): Promise<{ reslugged: number; conflicts: number }> {
  ...
  for (const row of rows) {
    ...
    // previous distribution, and chaining them would give `leaselab-copy-2-copy-2`.
  ...
  for (const [base, members] of groups) {
    ...
    for (const [index, member] of ordered.entries()) {
      ...
      /*
        The folder name usually already contains that of the project —`kiosk_new copy 12`
        contains `kiosk_new` — and prefixing it with the base path again produces
        `kiosk-new-kiosk-new-copy-12`, which is unreadable and does not distinguish any
      ...
      taken.add(slug);
      if (slug !== member.slug) assignments.push({ id: member.id, slug });
    }
  }
  ...
  return { reslugged: assignments.length, conflicts };
}
