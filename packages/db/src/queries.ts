 ...
 *
 * The separator is again ` `, which PostgreSQL does not support within a `text`, so neither of the
...
function observationId(row: { identity: string | null; statement: string }): string {
  return idFor([row.identity ?? "", row.statement].join(" "));
}
