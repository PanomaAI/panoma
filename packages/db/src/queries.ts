...
export interface PortfolioStats {
  ...
  copies: number;
  /**
   * Also outside `projects`, and for the other of the two reasons `notACopy` subtracts.
   *
   * The copies are set aside by the machine, and that is why they have always been counted here:
   * a number that hides what somebody else decided has to say so. These are set aside by the
   * person, one at a time, each with a notice that does not expire — so the day it happens nothing
   * is missing. What was missing was the month afterwards, with the sidebar three short and
   * nothing on any screen to remember it by.
   */
  hidden: number;
  redundantBytes: number;
  agentCommits: number;
...
export async function getStats(db: Database): Promise<PortfolioStats> {
  const [row] = await db
    .select({
      ...
      /*
        Counted from the decision and not from the project: `notACopy` reads `decisions.hidden`
        through `identity`, which is what survives moving the folder, and a decision can outlive
        the row it was about. Only rows that are still in the catalog are counted, so the figure
        can always be pointed at — and one that is also a copy is not counted twice.
       */
      hidden: sql<number>`(select count(*)::int from ${t.projects} p
        where not exists (select 1 from family_members fm where fm.project_id = p.id)
          and (select d.hidden from decisions d where d.identity = p.identity) is true)`,
