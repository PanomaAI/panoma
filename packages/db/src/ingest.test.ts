...
describe("una ingesta que se cae a mitad", () => {
  ...
  it("una ingesta que va bien sigue devolviendo lo mismo de siempre", async () => {
    ...
    expect(result).toEqual({
      ...
      removed: 0,
      excluded: 0,
      // Zero, and this line used to say two. Both projects were born in this same ingestion, so
      // neither had an address to lose — see `mudanzas` in `assignSlugs`.
      reslugged: 0,
  ...
  /*
    And the other half, which is the one that must not be lost in the fix.

    A project that really does move: `uno` is ingested, gets its address, and comes back under
    another name. The route is the same, so it is the same row — and its slug changes, which means
    a link that someone had stops working. That is exactly what the warning is for, and counting
    it is the reason the counter exists at all. A fix that silenced this one too would be worse
    than the bug it replaced: the first scan would stop lying, and every scan after would stop
    telling the truth.
   */
  it("y una mudanza de verdad sí se cuenta", async () => {
    const estreno = await ingestPortfolio(db, [analyses("uno")], [], ROOT);
    expect(estreno.reslugged, "nace, no se muda").toBe(0);

    const result = await ingestPortfolio(db, [{ ...analyses("uno"), name: "otro" }], [], ROOT);

    expect(result.reslugged, "se mudó de `uno` a `otro`").toBe(1);
    const [fila] = await db.select().from(t.projects).where(eq(t.projects.id, idFor(`${ROOT}/uno`)));
    expect(fila?.slug).toBe("otro");
  });
});
...
describe("un escaneo acotado no borra las familias de fuera", () => {
  /*
    The mistake, twice.
    The first: `panoma scan ~/Desktop/menucard --save` was analyzing a project, couldn't find
