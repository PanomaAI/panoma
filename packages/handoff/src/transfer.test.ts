...
import { FIXED_NOW, FIXTURE_CWD, FIXTURE_OPENCODE_ID, fixedRandom, layClaude, layCodex, layOpencodeStorage, opencodeRoot } from "./fixtures/index";
...
describe("handoff", () => {
  ...
  it("OpenCode: the envelope is written and the import step is returned, nothing is run", async () => {
    ...
    expect(result.path).toBe(join(opencodeRoot(h), `panoma-import-${result.sessionId}.json`));
    ...
    const source = await readConversation({ agent: "opencode", path: join(opencodeRoot(h), "opencode.db"), sessionId: FIXTURE_OPENCODE_ID }, { home: h, env: {} });
