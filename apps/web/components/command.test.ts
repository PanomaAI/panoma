...
describe("comandos que se pegan en un terminal", () => {
  it("deja en paz lo que no lo necesita", () => {
    expect(quoteForShell("/Users/ana/proyectos/panoma")).toBe("/Users/ana/proyectos/panoma");
  });

  it("entrecomilla los espacios, que son el caso real", () => {
    expect(quoteForShell("/Users/ana/brand assets/sundeck")).toBe(
      "'/Users/ana/brand assets/sundeck'",
    );
  });

  it("entrecomilla lo que el shell interpretaría", () => {
    expect(quoteForShell("/tmp/hølm copy 2")).toBe("'/tmp/hølm copy 2'");
  ...
  it("compone el comando entero", () => {
    expect(inFolder("/tmp/brand assets/app", "flutter run")).toBe(
      "cd '/tmp/brand assets/app' && flutter run",
    );
  });
...
describe("los mismos comandos, en PowerShell", () => {
  ...
  it("no usa && en ninguna parte", () => {
    ...
    expect(linea).toBe(
      "cd 'C:\\Users\\ana\\brand assets'; if ($?) { git init; git add -A }",
    );
  });
  ...
  it("una ruta de Windows normal no necesita comillas", () => {
    expect(quoteForShell("C:\\Users\\ana\\panoma", "powershell")).toBe(
      "C:\\Users\\ana\\panoma",
    );
  });
  ...
  it("una comilla simple se duplica, que es como se escapa allí", () => {
    ...
    expect(quoteForShell("C:\\Users\\ana's app", "powershell")).toBe(
      "'C:\\Users\\ana''s app'",
    );
  });
});
