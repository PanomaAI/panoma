      ...
      <span className="catalog-row__identity">
        ...
        <span className="catalog-row__names">
          ...
          {/*
            ...
            */}
          {!discreet && (
            <span className="catalog-row__path">
              {shortPath(project.root)}
              {/*
                 And what the row is standing for. The grid shows the canonical and keeps the
                 copies, so one line can be worth four folders without saying so — `copyCount` was
                 queried, sent to the browser and drawn nowhere. It goes here and not beside the
                 name because the two marks up there are for things that do not wait, and this one
                 does not press: it places.
                */}
              {project.copyCount > 0 && (
                <span
                  /*
                    No class and no colour of its own. The ink scale of this screen stops at
                    `--ink-3`, which the path already uses, and inventing a fainter step for an
                    aside is how a palette grows one token at a time. The «·» that opens the phrase
                    is the separation; the span is here to carry the explanation.
                   */
                  title={t("store.alsoCopiesTitle")}
                >
                  {" "}
                  {t("store.alsoCopies", { n: project.copyCount })}
                </span>
              )}
            </span>
          )}
