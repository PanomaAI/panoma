    ...
    <main id="app-main" tabIndex={-1} className="app-main">
      <section className="content-page empty-catalog-page">
        ...
        {/*
           ...
           And it carries `npx` in front for a reason that was hard to see: whoever came for
           `npx panoma up` **does not have `panoma` in the PATH** —npx runs from its cache and
           doesn't link anything—, so this same text would give them "command not found" at the
           first place where the product asks for something. With the prefix it works for both:
           npx resolves a binary from PATH without downloading anything if it is already
           installed.
          */}
