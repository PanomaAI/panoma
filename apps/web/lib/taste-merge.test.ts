...
describe("una creencia cuya línea está vieja porque la máquina la cambió", () => {
  ...
  /*
    ...
    was accepted as correct, and the `only in veloria:` was never written — nor was it ever
  ...
  it("acotarla escribe el «solo en» en el fichero", () => {
    ...
    expect(reconcileTaste(file, [row]).lines[0]?.scope).toBe("veloria");
  });

  it("y desacotarla lo quita", () => {
    const file: TasteLine[] = [
      { topic: "design", statement: "no uses degradados", citations: [V1], scope: "veloria" },
    ];
    const row: TasteStatement = {
      id: "b",
      topic: "design",
      statement: "no uses degradados",
      citations: [V1],
      published: { topic: "design", statement: "no uses degradados", scope: "veloria" },
    };
...
describe("las líneas de lo que ya no se publica", () => {
  ...
  it("el alcance entra en la comparación", () => {
    const file: TasteLine[] = [
      { topic: "design", statement: "una", citations: [], scope: "veloria" },
    ];
    expect(
    ...
    expect(
      dropStatements(file, [{ topic: "design", statement: "una", scope: "veloria" }]),
    ).toHaveLength(0);
  });
});
