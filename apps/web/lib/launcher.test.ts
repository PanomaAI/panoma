...
describe("las rutas no se pueden salir de su comilla", () => {
  it("una carpeta con espacios se entrecomilla entera", () => {
    ...
    const script = composeScript({ ...BASE, root: "/Users/x/brand assets/web app" });
    expect(script).toContain("cd '/Users/x/brand assets/web app' || exit 1");
  });
...
describe("abrir un agente sin encargo", () => {
  it("entra en la carpeta y ejecuta el agente, sin más argumentos", () => {
    const script = composeScript({
      root: "/Users/x/brand assets/app",
      command: "claude",
      args: [],
    });
    expect(script).toContain(`cd '/Users/x/brand assets/app' || exit 1`);
...
describe("el guion de PowerShell", () => {
  const base = {
    root: "C:\\Users\\ana\\mis proyectos\\panoma",
    command: "claude",
  ...
  it("no tiene shebang ni `exec`, que allí no significan nada", () => {
    ...
    expect(guion).toContain("Set-Location -LiteralPath 'C:\\Users\\ana\\mis proyectos\\panoma'");
  });
  ...
  it("lee el encargo del fichero y lo pasa como un solo argumento", () => {
    const guion = composeScript({ ...base, assignmentPath: "C:\\Users\\ana\\.panoma\\x.md" });
    expect(guion).toContain("(Get-Content -Raw -LiteralPath C:\\Users\\ana\\.panoma\\x.md)");
    /*
     ...
     */
    const veces = guion.split("C:\\Users\\ana\\.panoma\\x.md").length - 1;
    expect(veces).toBe(1);
  ...
  it("usa -LiteralPath, que no interpreta comodines", () => {
    ...
    const guion = composeScript({ ...base, root: "C:\\Users\\ana\\proyecto [viejo]" });
    expect(guion).toContain("Set-Location -LiteralPath 'C:\\Users\\ana\\proyecto [viejo]'");
  });
  ...
  it("escapa la comilla simple doblándola, que es como se hace allí", () => {
    ...
    const guion = composeScript({ ...base, root: "C:\\Users\\ana's app" });
    expect(guion).toContain("'C:\\Users\\ana''s app'");
