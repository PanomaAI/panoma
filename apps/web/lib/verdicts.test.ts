...
const CWD = "/Users/yo/Desktop/apuntes/apps/web";
...
describe("el cwd que no lleva a ningún proyecto", () => {
  ...
  it("los proyectos se cuentan sin repetir", () => {
    const otra = "/Users/yo/Desktop/quiet_mark/frontend";
    const mapa = new Map([
      [CWD, IDENTITY],
      [otra, "git:aa11bb22"],
    ]);
...
describe("la cita viaja tal cual, sin una segunda pasada de tapado", () => {
  ...
  const QUOTE =
    ...
    "mira /Users/yo/Desktop/apuntes/packages/core/src/history/claude-code.ts";
 ...
 * being talked about: in a real transcript, it said `arca89/quiet_mark` in 1,095 turns while the
 * files touched were 161 under `echoreel/` and one under `quiet_mark/`. The result was a sentence
 * about an audio tray, learned while working in `echoreel`, signed as from `Ledgerly` — and as
...
describe("de qué proyecto es una reacción", () => {
  const IDS = new Map([
    ["/w/arca89/quiet_mark", "git:quiet"],
    ["/w/arca89/echoreel/app/lib/audio.dart", "git:echo"],
    ["/w/arca89/echoreel/app/lib/feed.dart", "git:echo"],
  ...
  it("los ficheros tocados ganan al cwd", () => {
    const one = reaccion({
      cwd: "/w/arca89/quiet_mark",
      paths: ["/w/arca89/echoreel/app/lib/audio.dart", "/w/arca89/echoreel/app/lib/feed.dart"],
    });
    expect(identityOf(one, IDS)).toBe("git:echo");
  });

  it("sin ficheros tocados manda el cwd, como siempre", () => {
    expect(identityOf(reaccion({ cwd: "/w/arca89/quiet_mark" }), IDS)).toBe("git:quiet");
  });
  ...
  it("lo que no resuelve no vota", () => {
    const one = reaccion({
      cwd: "/w/arca89/quiet_mark",
      paths: ["/w/nada/a.ts", "/w/nada/b.ts", "/w/arca89/echoreel/app/lib/audio.dart"],
    });
    expect(identityOf(one, IDS)).toBe("git:echo");
  });
  ...
  it("en empate desempata el cwd, y el resultado no baila", () => {
    const one = reaccion({
      cwd: "/w/arca89/quiet_mark",
      paths: ["/w/arca89/echoreel/app/lib/audio.dart", "/w/arca89/quiet_mark"],
    });
    expect(identityOf(one, IDS)).toBe("git:quiet");
  ...
  it("cwd y ficheros se le preguntan al catálogo en la misma lista", () => {
    const rutas = distinctCwds([
      reaccion({ cwd: "/w/arca89/quiet_mark", paths: ["/w/arca89/echoreel/app/lib/audio.dart"] }),
    ]);
    expect(rutas).toContain("/w/arca89/quiet_mark");
    expect(rutas).toContain("/w/arca89/echoreel/app/lib/audio.dart");
  });
});
