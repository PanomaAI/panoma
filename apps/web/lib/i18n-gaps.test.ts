...
/**
 * `{cli}` is not filled by the caller either: it is put in by whoever renders.
 *
 * `useT()` injects it into every call from the context that `I18nProvider` carries down, exactly
 * as it does with the language, and for the same reason — a gap that a hundred call sites have to
 * remember is a gap some call site will forget, and this one fails without a mark: the sentence
 * still renders, it just names a command the reader does not have on their PATH.
 *
 * Only the calls with the key in the first argument reach this test —the two-argument form, which
 * is the one `useT()` gives— so every call it looks at is one that gets `{cli}` injected. The
 * server ones, `t(locale, key, {…})`, pass it by hand and are checked by the compiler instead:
 * `cliName()` has no default and there is no locale-less overload to fall into.
 */
const RELLENADOS_POR_QUIEN_PINTA = new Set(["cli"]);

function huecos(texto: string | undefined): string[] {
  return [...(texto ?? "").matchAll(/\{(\w+)\}/g)]
    .map((m) => m[1]!)
    .filter((nombre) => !esHuecoDeForma(nombre) && !RELLENADOS_POR_QUIEN_PINTA.has(nombre))
    .sort();
}
