...
describe("dónde vale y dónde podría valer", () => {
  const names = { "git:uno": "veloria", "git:dos": "echoreel" };
  const identities = { veloria: "git:uno", echoreel: "git:dos" };

  it("se resuelve el nombre desde la identidad", () => {
    expect(asBelief(row({ identity: "git:uno" }), { names }).scope).toBe("veloria");
  });
  ...
  it("con evidencia de dos proyectos no se ofrece acotar a ninguno", () => {
    const belief = asBelief(
      row({ citations: [CITA, { ...CITA, verdictId: "v2", project: "echoreel" }] }),
      { names, identities },
    );
  ...
  it("una ya acotada no trae candidato", () => {
    ...
    expect(belief.scope).toBe("veloria");
    expect(belief.learnedIn).toBeUndefined();
  });
});
