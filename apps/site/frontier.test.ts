...
/*
  Every path this file compares is written with `/`: the regex that finds the API routes, the list
  of the permitted ones, and the prefix that decides whether an import stays inside. Windows writes
  them with `\`, so without normalizing at the door the regex matched nothing —the permitted route
  looked deleted— and every relative import looked like it escaped the border. Fifty-six false
  leaks and a guard reporting the opposite of the truth.

  Normalizing once here and not case by case is deliberate: the alternative is remembering it at
  five call sites, and the sixth is the one that comes back.
 */
const aPosix = (ruta: string) => ruta.split(sep).join("/");
const raizSitio = aPosix(fileURLToPath(site)).replace(/\/$/, "");
...
describe("el sitio público no cruza la frontera", () => {
    ...
    for (const { ruta, test } of todos) {
      ...
      for (const spec of importsDe(ruta)) {
        if (spec.startsWith("@panoma/")) {
          fugas.push(`${aPosix(relative(raizSitio, ruta))} → ${spec}`);
          continue;
        }
        if (!spec.startsWith(".")) continue;
        const destino = aPosix(resolve(dirname(ruta), spec));
  ...
  it("no hay más rutas de API que las decididas", () => {
    const rutas = todos
      .filter(({ ruta }) => /\/app\/.*\/route\.tsx?$/.test(aPosix(ruta)))
      .map(({ ruta }) => aPosix(relative(raizSitio, ruta)));
