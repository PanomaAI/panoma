...
export default function config(phase: string): NextConfig {
  ...
  return {
    ...
    /*
      ...
      including `fileURLToPath`, which is not decoration: `.pathname` is a URL component, and on
      Windows it answers `/D:/…` with a slash in front of the drive letter. `apps/web` died on
      exactly that when the package was first built there, tracing into the user's home directory
      until a legacy junction refused to be read. Vercel builds this one on Linux, where the two
      strings agree and the bug cannot show — which is the whole reason it deserves fixing now
      rather than the day someone builds the site on a laptop that runs Windows.
     */
