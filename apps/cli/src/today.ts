...
export function period(since: string | null, now: string): string {
  // No mark at all is the first run, not a long absence: `visit.json` is written only after a
  // look, so the report has nothing to compare against and opens a window of one day. Saying
  // "since you last looked" there was the first sentence a new user read, and it was false.
  if (!since) return say("today.firstLook");

  const start = new Date(since);
  const fin = new Date(now);
  if (Number.isNaN(start.getTime()) || Number.isNaN(fin.getTime())) {
    // A mark that exists and cannot be read is a different case: somebody did look before, so the
    // vague sentence is still true. Only its absence means nobody has.
