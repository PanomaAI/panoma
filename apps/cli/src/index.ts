...
async function main(): Promise<number> {
  ...
  /*
    ...
    The route departs from the second position, as in `scan`: `panoma review ~/Desktop/kestrel`.
   */
  if (command === "review") {
  ...
  if (analyses.length === 1) {
    ...
    const hint =
      totalCopies > 0
        ? pc.yellow(say("scan.looksLikeCopies", { n: totalCopies }))
...
async function saveToCatalog(
  ...
  if (result.removed) {
    process.stderr.write(pc.dim(`  ${say("cli.removed", { n: result.removed })}\n`));
  }
  if (result.excluded) {
    process.stderr.write(
      pc.dim(`  ${say("cli.excluded", { n: result.excluded })}\n`),
    );
  }
...
async function enrichCatalog(api: string, force: boolean): Promise<number> {
  ...
  process.stdout.write(
    [
      ...
      `      ${say("enrich.outdated", { n: pc.yellow(String(result.outdated)) })}`,
      result.advisories > 0
        ? `      ${pc.red(say("enrich.advisories", { n: result.advisories }))} ${say("enrich.affect", { n: result.vulnerablePackages, s: plural(result.vulnerablePackages) })}`
