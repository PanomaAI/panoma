...
describe("los flags de los verbos nuevos", () => {
  it("open elige herramienta con --folder y --terminal", () => {
    expect(flagsOf(["open", "kestrel", "--folder"]).folder).toBe(true);
    expect(flagsOf(["open", "kestrel", "--terminal"]).terminal).toBe(true);
    // The name of the project is a positional, not the value of any flag.
    expect(flagsOf(["open", "kestrel", "--terminal"]).positionals).toEqual(["open", "kestrel"]);
  });
