...
async function mine(parsed: Flags): Promise<number> {
  ...
  if (project !== undefined) {
    ...
    // compared: saying 'starts with "~/Desktop/apuntes"' when what was compared was the absolute one
 ...
 * `--project "~/Desktop/apuntes"` —quoted in zsh, or any invocation from PowerShell that doesn’t
...
export function corpusLines(corpus: CorpusProgress | undefined): string[] {
  ...
  if (left === 0) return [`  ${pc.dim(say("twin.corpusDone", { total: corpus.total }))}`];
  return [
...
export interface TasteReply {
  ...
  /** From identity to project name. Without this, you cannot say 'only in veloria'. */
  names?: Record<string, string>;
}
  ...
  for (const topic of topicsOf(rows)) {
    ...
    for (const row of rows.filter((one) => one.topic === topic)) {
      ...
      /*
        ...
        reach exists: the file is written —`only in veloria:`— and this screen threw it when
