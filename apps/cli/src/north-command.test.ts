...
describe("qué se ha pedido", () => {
  ...
  it("con un proyecto, su norte", () => {
    expect(northArgs(["kestrel"])).toEqual({ mode: "show", slug: "kestrel" });
  });

  it("con una frase detrás, la escritura", () => {
    expect(northArgs(["kestrel", NORTH])).toEqual({
      mode: "write",
      slug: "kestrel",
      phrase: NORTH,
    });
  ...
  it("la frase escrita sin comillas llega entera y no partida", () => {
    ...
    const args = northArgs(["kestrel", "Terminado", "=", "que", "compile"]);
    expect(args).toEqual({ mode: "write", slug: "kestrel", phrase: "Terminado = que compile" });
  });
