...
`veloria` showed up in the catalog as `mapbox-maps-flutter-main`, which is neither what its
...
1. **`shadowed`**: a child is named the same as the container (`kestrel/kestrel`). That is not a
...
and date — the three strong signals, all three at once. Without this exit, `veloria` and the
landing page ended up flagged as copies of `kestrel`: eleven unrelated projects in a single
