...
(`flutter`), and behind it come 117 (`PORTAL`) and 97 (`brand assets`). With 600 you do
...
The case that rules is `pocket_bot`, whose entire palette is this:
...
- `apps/web` in this repository: `8px` eighteen times, `7px` fourteen, `6px` seven, `5px`
  ...
  sitting.
- `pocket_bot`: `15px` repeated across every chat bubble and one stray `16.0px` in a form.
...
than zero**: in `pocket_bot` a `10px` from a stylesheet coexists with a `10.0px` from a Dart
...
measured on `quiet_mark/frontend`, the fingerprint came out truncated, the two drifts went
