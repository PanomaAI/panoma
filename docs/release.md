...
And there is a ninth manifest that does not belong to the monorepo and is worth not confusing
And a warning for whoever publishes for the first time: **the name `panoma` is already taken
on npm**. A reservation holds it — version `0.0.1`, from 16-Aug-2026, a three-file package
whose `index.js` prints "coming soon" and leaves — made so nobody could take the name while
the real CLI was arriving. Its code does not travel in this repository, because it is not
product. The consequence does matter: the first real publish **replaces that reservation
rather than christening the name**, so the account already has permission and whatever version
ships has to be greater than `0.0.1`.
...
**And the clone comes from the public repository**, `panomahq/panoma`, not from the tree you
...
- **Nothing is written about versioning or about how a bad version is pulled.** The monorepo's
  eight manifests are all still at `0.1.0`, so the first time two releases in a row are needed,
  what is missing here will be discovered.
