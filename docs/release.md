...
**And the mirror gets an annotated tag, `vX.Y.Z`, which is not bookkeeping.** Pushing it is what
starts `package.yml`, the only check in the project that installs what was actually published and
walks it on the three systems; `tests.yml` runs on every push and asks a different question. A
release without its tag is a release nobody verified anywhere but the machine that packed it. The
tag also holds its commit reachable for good, which is what `BUILD-INFO.json` promises whoever
wants to read the source of the version they are running.

```bash
git -C ~/panoma-oss tag -a v0.1.4 -m "panoma 0.1.4" -m "..."
git -C ~/panoma-oss push origin v0.1.4
```
...
- **The tags live in the mirror and nowhere else.** `git tag` in the working tree answers nothing,
  which is misleading rather than merely incomplete: it reads as «this project does not tag», and
  the four that exist are on `PanomaAI/panoma`. Whoever looks for the history of a release has to
  know to look in the other repository — this page is the only place that says so.
