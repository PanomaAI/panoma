...
the stylesheet on 8-Sep-2026, those on the handoff on 11-Sep-2026 and those on its agent
channel on 12-Sep-2026.
...
| The twin's Codex reader looks for `event_msg/user_message`, and Codex no longer writes it here | `packages/core/src/history/codex.ts` | Measured on 12-Sep-2026 while re-keying the handed-off skip: none of the 315 rollouts under `~/.codex/sessions` on this disk carries that event any more — a `rollout-migrations` folder exists, and the person's turns live as `item_completed` events with `item.type: "UserMessage"` — so the twin mines nothing from Codex on this disk today. The handoff's own reader (`packages/handoff/src/readers/codex.ts`) reads `response_item` messages and is not affected. Teaching the twin's reader the new shape is a change to what it counts as owner evidence, which is the twin's decision record to amend | **the user** |
...
- **The figures are from 25-Aug-2026, those of the spend and dependency tables from 6-Sep-2026,
  ...
  table from 11-Sep-2026 and its three agent-channel rows from 12-Sep-2026**, and were checked by
