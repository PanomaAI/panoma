...
go stale here; on 12-Sep-2026 it named route files to the number of twenty-four, and on
14-Sep-2026, after the two memory deliveries, thirty-four — two of which, `memory/export` and
`memory/status`, name it only to say they do not cut. The memory worker's free and paid passes
— the receipt reader, the capture pass and the project extraction — sit behind the same
variable in `apps/web/lib/memory-worker.ts`, because they open the person's disk and spend the
server's key ([memory-capture.md](memory-capture.md)). Delivery B added no variable of its own:
its caps are the `memory` family's below, and its budgets are constants in their modules.
Delivery E added two, the storage quotas `PANOMA_MEMORY_QUOTA_MB` and `PANOMA_PROJECT_QUOTA_MB`,
listed with the caps below because they follow the same precedence and the same screen.
...
| `PANOMA_SIGNAL_V2` | `signalCommand` in `apps/cli/src/signal.ts` | unset: the legacy `GET /api/agent/notes`, byte for byte | Only the exact value `1` opens the v2 road of the edit hook — `POST /api/hook/context` with the channel `signal`, the memory contract of that path printed as the server rendered it — with the fallback to the legacy `GET` on `409 unsupported_host` or a server without the route, inside the same two seconds; any other value leaves the road closed. In delivery A the server answers `unsupported_host` on that channel for every host, because no native ceiling has been measured, so the switch is a test seam more than a setting. See [memory-contract.md](memory-contract.md). |
...
| `PANOMA_DISTILL_BUDGET` | `capFor("memory")` in `apps/web/lib/spend-settings.ts`, asked by `apps/web/lib/memory-distill.ts` and, since delivery B, by `apps/web/lib/memory-extract.ts`; the family is called `memory` and the variable keeps its historical name | 12 calls a day, distillations and extractions together | The organ that re-reads a closed visit and proposes what is durable, and the paid extraction of the owner's own turns, which takes at most `min(4, cap)` automatic calls a day inside the same number. Both run in the background, so on overflow there is no error to show: the distiller returns `{ did: "budget" }` and calls nobody, and the extractor defers its window to tomorrow. A first answer cut by the output ceiling that cannot be read is asked for once more, so one session may cost two of the day's calls. With `0`, memory only grows with what somebody writes on purpose. |
...
| `PANOMA_HANDOFF_BUDGET` | `capFor("handoff")` in `apps/web/lib/spend-settings.ts`, asked by `app/api/handoff/route.ts` when the body says `digestBy: "model"` and by `app/api/handoff/digest/route.ts` | 10 calls a day | The model-written summary of a conversation about to be handed off: since 15-Sep-2026 one call per window of 60,000 rendered characters of the transcript, planned before anything is paid, plus one for each answer that was cut and asked again; until then one call per handoff. It is the one call that sends a whole private transcript —redacted and wrapped— to a provider, which is why the cap is the smallest of the nine and why the box on the screen starts off unless the tier needs a digest, a model is connected, the chain fits in what is left today and the source carries no summary panoma can read. Before any call both routes check the whole chain against the cap and answer `429` with `{ error, hint }` — with nothing left, the hint names the Spend screen and this variable; with fewer calls left than the chain needs, the body adds `needs` and `left` and the hint names the Spend screen or the mechanical digest — and nothing is written: without the box the same handoff goes through with the mechanical digest, which costs nothing ([budgets.md](budgets.md)). With `0`, the box on the screen is disabled and `--digest model` is refused before anything is written. The agent channel never asks for it: `POST /api/agent/handoff` has no `digestBy` and refuses a body that carries one, so nothing an agent orders counts here. |
| `PANOMA_MEMORY_QUOTA_MB` | `memoryQuota()` in `apps/web/lib/spend-settings.ts`, read by the worker's gate (`lib/memory-quota.ts`), the capture pass, the offer and every writer of charged content through the limits it hands them | 256 MiB | Not a call cap: the logical bytes of derived memory the whole catalog may keep before every automatic pass pauses and an automatic write is refused; the owner's own gestures are charged and never refused, and nothing is deleted to make room. The precedence is the variable → `quota.catalogMb` in `spend.json` → the factory value, without the pause. Zero, a negative number, a fraction or anything over a tebibyte falls to the factory value with the environment still named as the source, and the Spend screen shows the input disabled with the variable named ([budgets.md](budgets.md), [memory-capture.md](memory-capture.md), «The storage quota»). |
| `PANOMA_PROJECT_QUOTA_MB` | the same `memoryQuota()`, for the per-project counter | 64 MiB | The same bytes, per project: a project at its limit is skipped by the passes and its jobs wait while the rest of the catalog goes on. Same precedence (`quota.projectMb` in the file) and the same refusals. |
...
| `twin.json` | The permission for each history source and the twin's only question (`inferred`); since 14-Sep-2026 also `grants`, the capture permissions on top of a source's yes — purpose, scope, generation, notice version and activation instant — written only when at least one exists, so a file that never held a grant keeps its old shape. JSON with two spaces and the identifier untranslated, so you can read it with `cat` and revoke it with `rm`. | No, and that is the point: the absence of an answer is **not** a yes. |
| `TASTE.md` | The portrait: the sentences of your taste that come down to all your agents, capped at 3,000 characters. Plain text, editable with any editor. | It is rewritten on every `POST /api/twin/taste`. The file **is the input**: deleting a line vetoes that belief and rewriting it signs it in the new words, and since 14-Sep-2026 both gestures are applied before a criterion is delivered to any agent, not only when that route runs; a file that cannot be read leaves the criteria out of that delivery, and an absent one withdraws nothing ([memory-contract.md](memory-contract.md)). |
...
| `signal-seen.json` | Which sleeping notes were handed to each session, so as not to re-inject the same signal on every edit under its zone. At most 20 sessions. The legacy road's record only: on the v2 road what a context has seen lives in the catalog. | Yes. It is the only file that builds its path by hand instead of with `panomaPath`, though it honors `PANOMA_HOME` all the same. |
| `memory-deletions.jsonl` | The journal of forgetting: a header line with the journal's id, then one line per withdrawal or purge with its sequence, its operation and opaque ids only — never a path, a phrase or a hash of one. `0600`, appended and fsync'd **before** the operation's row exists in the catalog, so a database backup taken before a purge cannot bring the text back in silence. At every start the catalog compares it with `memory_deletions`. | No, and its absence is the point: a missing, torn or foreign journal puts the memory in **quarantine** — no delivery, no capture, no forgetting — until a person reconciles it; `panoma memory status` names the reason. Restore it together with `db/`, or accept the quarantine. See [memory-contract.md](memory-contract.md). |
