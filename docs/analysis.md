...
called `quiet_mark` whose README opens with "# Ledgerly" **is called Ledgerly**, and
cataloging it as `quiet_mark` meant its own author could not find it. `readmeName` takes that
...
`brand assets`, which talks about `sundeck`, does not steal the name from the child that
carries it.
...
- `qualifyWithParent(name, padre)` applies **wherever the name came from**: if it is in
  ...
  apart, and now one is "deskbot backend".
- `qualifyWithFolder(name, folder)` only when the name came from the README, and only if it is
  ...
  of `echoreel/server` says "EchoReel" and was cataloged the same as the app; "EchoReel
...
which is the canonical one. Sorting by longest name, `kestrel` showed up in the catalog with
...
proof, `leaselab-app-mobile-develop` —the user's own, with two sibling folders of the same
